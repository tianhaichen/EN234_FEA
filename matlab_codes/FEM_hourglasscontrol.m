...
%          Demonstrates use of hourglass control to stabilize
%          underintegrated 2D quadrilateral elements
%
...
%====================== Hourglassbasevectrs ===============================
%
function G = hourglassbasevectors(ncoord,nelnodes)
...
%   Compute the hourglass shape vectors
...
%
      g = hourglassbasevectors(ncoord,nelnodes);

      for i  = 1 : ncoord
        xi(i) = 0.;
        for a  = 1 : nelnodes
          xi(i) = xi(i) + g(a)*coord(i,a);
        end
      end
...
%     Correct the base vectors to ensure orthogonality
      ...
      for a  = 1 : nelnodes
        for i  = 1 : ncoord 
          g(a) = g(a) - xi(i)*dNdx(a,i);
          scale = scale + dNdx(a,i)*dNdx(a,i);
        end
      end
        ...
        for i  = 1 : ndof
          for b  = 1 : nelnodes
            for k  = 1 : ndof
              row = ndof*(a-1)+i;
              col = ndof*(b-1)+k;
              kel(col,row) = kel(col,row) + 0.01*scale*g(a)*g(b)*dt*4;
            end
          end
        end
      end
