...
%          Plots first 4 vibration modes for an elastic solid.
...
%  Fix constrained nodes. 
 ...
 %
 %  Set up matrices to do modal dynamics
 %
 rootM = sqrtm(M);
 ...
 %
 for i =1:ndof*nnode elist(i) = Lambda(i,i); end
 elist = sort(elist);
 %
 ...
 %
   for i = 1 : nnode*ndof
     if ((Lambda(i,i) - elist(mode+3*(ndof-1)))^2 < 0.00000001) 
       mn = i;
     end
   end
